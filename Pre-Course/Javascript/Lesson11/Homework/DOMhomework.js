        ...
        // the second is a numerical index.
          ...
          addToDo: function(data){
            ...
            if(Data.toDos.length >= 0){
                // console.log( Data.toDos.length -1 + 1)
                ID = Data.toDos.length -1 + 1
            } else {
                ID = 0
            }
            // toDoCreation
            console.log('Item Created')
            newItem = new ToDos(ID, data)
  ...
  return {
      getDOMStrings: function(){
          let DOM = {
                ...
                toDos: document.querySelector('.task')
            
          }
          return DOM;
        },
        displayNewLists: function(data, id){
            let resultsLists = `<li class="task" id="todo-${id}" data-taskid="${id}">
                                   ${data} 
                                   <span class="del-toDo">X</span>
                                </li>`                   
          this.getDOMStrings().toDoLists.insertAdjacentHTML('beforeend', resultsLists);
        },
...
var mainController = (function(toDoCtrls, UICtrls){
  ...
  var eventListeners = function(){
    ...
    document.addEventListener('keypress', function(e){
        if(e.keyCode === 13){
          addItemCtrls()
          UICtrls.getDOMStrings().userInput.value = "";
  ...
  var addItemCtrls = function(){
    if(UICtrls.getDOMStrings().userInput.value !== ""){
      //get input from UI
        let userData = UICtrls.getDOMStrings().userInput.value; 
      // //add input to the array
      let data = toDoCtrls.addToDo(userData)
      // render UI
      UICtrls.displayNewLists(data.description,data.id)
    }
  }
